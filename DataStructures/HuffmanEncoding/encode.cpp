//
// Created by gignt on 07.12.2023.
//
#include <bits/stdc++.h>
...
std::map<char, std::string> generate_codes(const std::shared_ptr<HuffmanNode>& tree) {
    ...
    std::queue<std::pair<std::string, std::shared_ptr<HuffmanNode>>> queue;
    queue.push({"", tree});
    while (!queue.empty()) {
        auto currentPair = queue.front();
        queue.pop();
        auto currentNode = currentPair.second;
        auto code = currentPair.first;
        if (currentNode->left != nullptr) {
            queue.push({code + '0', currentNode->left});
            queue.push({code + '1', currentNode->right});
        } else {
...
int main(){
    ...
    std::map<char,int> freq_map = count_freqs(file);
